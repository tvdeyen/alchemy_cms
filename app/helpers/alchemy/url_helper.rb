...
module Alchemy
  module UrlHelper
    ...
    def show_page_path_params(page, optional_params = {})
      ...
      url_params = { url_path: page.url_path }.update(optional_params)
    ...
    def download_alchemy_attachment_path(attachment)
      alchemy.download_attachment_path(attachment, attachment.url_path)
    end

    # Returns the url for downloading an alchemy attachment
    def download_alchemy_attachment_url(attachment)
      alchemy.download_attachment_url(attachment, attachment.url_path)
    end

    # Returns the full url containing host, page and anchor for the given element
    def full_url_for_element(element)
      "#{current_server}/#{element.page.url_path}##{element_dom_id(element)}"
    end
  end
end

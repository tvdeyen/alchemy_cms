...
Vue.component('alchemy-essence-file', {
  props: {
  ...
  computed: {
    ...
    attachment() {
      const element = this.$store.state.elements.find(element => element.id === this.content.element_id),
            content = element.contents.find(content => content.id === this.content.id);
